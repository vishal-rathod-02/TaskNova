...
// Edit / Delete flow (same as TasksView)
...
const confirmDelete = async () => {
  ...
  try {
    ...
    error.value = requestError.response?.data?.message || "Unable to delete task.";
  } finally {
...
onMounted(async () => {
  ...
  if (!projectStore.items.length) {
    try {
      ...
      // projects optional for edit modal dropdown
    }
  }
});
</script>
    ...
    <div v-else class="mt-6 space-y-4">
      ...
      >
        ...
        >
            ...
            <div class="flex min-w-0 flex-1 items-center gap-3">
              ...
              <div class="min-w-0 flex-1">
                <div class="flex flex-wrap items-center gap-2">
                  <h3 class="break-words font-display text-base font-bold text-slate-900 dark:text-white">
                    {{ item.title }}
                  </h3>
          ...
          <!-- Card Action Buttons -->
          <div class="mt-4 flex flex-col gap-2.5 border-t border-slate-200/60 pt-3 dark:border-slate-800/80 sm:flex-row sm:flex-wrap sm:items-center sm:justify-between">
            <button
              type="button"
              class="btn-primary w-full justify-center gap-1.5 text-xs shadow-sm sm:w-auto"
              @click="openDailyDigest(item)"
            >
              <AppIcon name="sparkles" :size="14" class="flex-none" />
              <span class="truncate">View Full Digest & Breakdown &rarr;</span>
            </button>
        ...
        >
          ...
          <div class="flex w-full flex-wrap items-center justify-end gap-2 self-stretch border-t border-slate-100 pt-2 sm:w-auto sm:self-center sm:border-t-0 sm:pt-0">
            <button
              v-if="item.task_id"
              type="button"
