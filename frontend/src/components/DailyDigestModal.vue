      ...
      >
        ...
        >
          ...
          <div v-else class="max-h-[75vh] overflow-y-auto p-5 sm:p-6 space-y-6">
            ...
            <div class="min-w-0">
              <div class="-mx-5 flex items-center gap-2 overflow-x-auto border-b border-slate-200/80 px-5 pb-2 dark:border-slate-800 sm:mx-0 sm:px-0">
                <button
                  type="button"
                  class="min-h-9 flex-none rounded-xl px-3 py-1.5 text-xs font-bold transition-all"
                ...
                >
                  Courses ({{ digest?.courses?.length || 0 }})
                </button>

                <button
                  type="button"
                  class="flex min-h-9 flex-none items-center gap-1.5 rounded-xl px-3 py-1.5 text-xs font-bold transition-all"
                ...
                >
                  <span>Due</span>
                  <span v-if="digest?.urgent_tasks?.length" class="rounded-full bg-rose-100 dark:bg-rose-950 px-1.5 py-0.5 text-[10px] text-rose-700 dark:text-rose-300">
                ...
                >
                  Done ({{ digest?.recent_completed?.length || 0 }})
                </button>
              </div>
              ...
              <div v-if="activeTab === 'courses'" class="mt-3 space-y-2.5">
                ...
                >
                  <div class="mb-1.5 flex flex-col gap-1 text-xs sm:flex-row sm:items-center sm:justify-between">
                    <span class="flex min-w-0 items-center gap-2 font-bold text-slate-900 dark:text-white">
                      <span class="h-2.5 w-2.5 flex-none rounded-full" :style="{ backgroundColor: course.color || '#4f46e5' }"></span>
                      <span class="min-w-0 truncate">{{ course.name }}</span>
                      <span class="hidden flex-none font-mono text-[10px] uppercase text-slate-400 sm:inline">[{{ course.code }}]</span>
                    </span>
                    <span class="flex-none font-mono font-bold text-slate-600 dark:text-slate-300">
          ...
          <div class="flex flex-col gap-2.5 border-t border-slate-200/80 bg-slate-50/80 px-4 py-4 dark:border-slate-800 dark:bg-night-card/80 sm:flex-row sm:flex-wrap sm:items-center sm:justify-between sm:px-6">
            <button
              type="button"
              class="btn-secondary w-full justify-center gap-1.5 text-xs sm:w-auto"
              @click="printDigest"
            >
            ...
            <div class="grid grid-cols-2 gap-2 sm:flex sm:items-center sm:gap-2">
              <button
                type="button"
                class="btn-primary justify-center gap-1.5 text-xs"
                @click="goToTasks"
              >
                <AppIcon name="tasks" :size="15" class="flex-none" />
                <span class="truncate">Task Ledger</span>
              </button>

              <button
                type="button"
                class="btn-ghost min-h-10 justify-center text-xs"
                @click="emit('close')"
              >
                Close
              </button>
